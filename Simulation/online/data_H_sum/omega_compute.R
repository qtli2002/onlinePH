...
#nlambda
p<-400
path <- ("D:/1mat/simulation/code/online/")
pathname <- file.path(path, "H_ave.mat")
H<-readMat(pathname)
H_compute<-H$H.ave
#If p = 200, set lambda.min = 1e-6 and nlambda = 800; If p = 400, set lambda.min = 1e-5,nlambda = 120.
...
omega<-out2[["icov"]]
writeMat("D:/1mat/simulation/code/online/data_H_sum/omega.mat",omega = omega)